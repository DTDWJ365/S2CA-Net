...
class SubjectReader:
    ...
    def get_evaluation_transform(inference=False):
        ...
        if inference:
            load = monai.transforms.LoadImaged(keys=image_keys)
            pad = monai.transforms.SpatialPadd(keys=image_keys, spatial_size=(240, 240, 160))
        else:
            load = monai.transforms.LoadImaged(keys=training_keys)
            pad = monai.transforms.SpatialPadd(keys=training_keys, spatial_size=(240, 240, 160))
        if inference:
            preprocess = monai.transforms.Compose([
                load,
                AddChanneld(keys=image_keys),
                pad,
                monai.transforms.NormalizeIntensityd(keys=image_keys),
                monai.transforms.ToTensord(keys=image_keys)
            ])
        else:
            preprocess = monai.transforms.Compose([
                load,
                ...
                pad,
                monai.transforms.MapLabelValued(keys='label', orig_labels=(0, 1, 2, 4), target_labels=(0, 1, 2, 3)),
                monai.transforms.NormalizeIntensityd(keys=image_keys),
                monai.transforms.ToTensord(keys=training_keys)
            ])
        return preprocess
    ...
    def get_training_transform(self):
        ...
        training_keys = ('t1', 't2', 't1ce', 'flair', 'label')  # BraTS training set contains label
        image_keys = ('t1', 't2', 't1ce', 'flair')
        augment = monai.transforms.Compose([
            monai.transforms.LoadImaged(keys=training_keys),
            AddChanneld(keys=training_keys),
            monai.transforms.MapLabelValued(keys='label', orig_labels=(0, 1, 2, 4), target_labels=(0, 1, 2, 3)),
            monai.transforms.RandSpatialCropd(keys=training_keys, roi_size=self.training_size, random_size=False),
            monai.transforms.NormalizeIntensityd(keys=image_keys),
            monai.transforms.RandAxisFlipd(keys=training_keys, prob=0.5),
            monai.transforms.RandScaleIntensityd(keys=image_keys, factors=0.1, prob=0.5),
            monai.transforms.RandShiftIntensityd(keys=image_keys, offsets=0.1, prob=0.5),
            monai.transforms.Rand3DElasticd(keys=training_keys, prob=0.5, mode='nearest',
                                            sigma_range=(1, 20), magnitude_range=(0.3, 2.3),
                                            rotate_range=(np.pi / 6, np.pi / 6, np.pi / 6),
                                            shear_range=(0.1, 0.1, 0.1),
                                            scale_range=(0.2, 0.2, 0.2)),
            monai.transforms.EnsureTyped(keys=training_keys)
        ])
        return augment
