...
def parse_args():
    ...
    parser.add_argument('--model', type=str, default='s2ca_net',
    ...
    parser.add_argument('--root', default="./dataest/BraTS2018/MICCAI_BraTS_2018_Data_Training",
                            help='train dataset path')
    parser.add_argument('--model_save_path', default='./dataset/model_pth', type=str, help='resume training')
    parser.add_argument('--ncpu', type=int, default=4, help='number of workers for dataloader')   #加载数据的速度
...
def main():
    ...
    for epoch in range(start_epoch + 1, num_epochs + 1):
        ...
        if not use_trainset and epoch % eval_interval == 0:
            ...
            model.eval()
            with torch.no_grad():
                val_iter = tqdm.tqdm(val_loader)
                for step, batch_data in enumerate(val_iter):
                    inputs_t1 = batch_data['t1'].to(device)
                    inputs_t2 = batch_data['t2'].to(device)
                    inputs_t1ce = batch_data['t1ce'].to(device)
                    inputs_flair = batch_data['flair'].to(device)
